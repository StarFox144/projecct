...
async function createExchangeAndQueues(ch) {
    try {
        // Створення обмінника
        await ch.assertExchange('StarFox', 'direct', { durable: true });

        // Створення черг для користувачів і турів
        await ch.assertQueue('FoxQueue', { durable: true });

        // Зв'язування черг з обмінником
        await ch.bindQueue('FoxQueue', 'StarFox', ''); // Вказуйте тут ім'я черги та обмінника, які ви використовуєте
